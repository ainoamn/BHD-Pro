  ...
  return (
    <div className="space-y-6">
      ...
      <GlassCard className="p-4 flex items-center justify-between gap-3">
        ...
        <p className="text-lg font-semibold text-slate-900 dark:text-white">{year}</p>
        <button
          onClick={() => setYear((y) => y + 1)}
          className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white"
        >
            ...
            {periods.map((p) => (
              <GlassCard key={p.id} className="p-4 space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="text-slate-900 dark:text-white font-medium">{t(MONTH_KEYS[p.month - 1] as "m1")}</p>
                ...
                {p.isLocked ? (
                  isAdmin && (
                    <button
                      ...
                      className="text-xs px-3 py-1.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 inline-flex items-center gap-1"
                    >
          ...
          <GlassCard className="hidden md:block overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {periods.map((p) => (
                    <tr key={p.id} className="border-b border-slate-200 dark:border-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800/30">
                          ...
                          {p.isLocked ? (
                            isAdmin ? (
                              <button
                                disabled={pending}
                                onClick={() => setUnlockMonth(p.month)}
                                className="text-xs px-3 py-1.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 inline-flex items-center gap-1 hover:text-slate-900 dark:hover:text-white"
                              >
                                <Unlock className="w-3 h-3" />
                                {t("unlock")}
                              </button>
